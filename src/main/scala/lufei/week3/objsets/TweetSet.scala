package lufei.week3.objsets

import lufei.week3.objsets.TweetReader.*
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  override def union(that: TweetSet): TweetSet =
    NonEmpty(elem, left.union(that.filter(elem.text > _.text)), right.union(that.filter(elem.text < _.text)))

  override def mostRetweeted: Tweet = (left, right) match
    case (_: Empty, _: Empty) => elem
    case (l: NonEmpty, _: Empty) => compareTweets(elem, l.mostRetweeted)
    case (_: Empty, r: NonEmpty) => compareTweets(elem, r.mostRetweeted)
    case (l: NonEmpty, r: NonEmpty) => compareTweets(elem, compareTweets(l.mostRetweeted, r.mostRetweeted))

  private def compareTweets(x: Tweet, y: Tweet): Tweet =
    if x.retweets < y.retweets then y else x

  override def descendingByRetweet: TweetList =
    val mostTweet = this.mostRetweeted
    Cons(mostTweet, this.remove(mostTweet).descendingByRetweet)
